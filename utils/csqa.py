...
def get_numpy_ev(line_ids, batch_size, def_val = "NOENT"):
    assert batch_size == len(line_ids), ('batch sizes not equal', len(line_ids), batch_size)
    max_seq_len = np.max([len(each_ex) for each_ex in line_ids])
    ent_vecs_list = []
    
    for _idx_ex in range(len(line_ids)):
        ent_vecs=[def_val]*(max_seq_len+1)
        # print("\n\n\n111111111111111111111111111", ent_vecs ,"\n", line_ids[_idx_ex] )
        for _idx_t, _id in enumerate(line_ids[_idx_ex]):
            ent_vecs[_idx_t] = _id
        ent_vecs_list.append(ent_vecs)
        # print("\n\n\n", ent_vecs ,"\n", line_ids[_idx_ex])

    return ent_vecs_list
