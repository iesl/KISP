...
from utils.csqa import get_data_path_list, load_json, save_json, get_numpy_ev, first_nonzero
from collections import defaultdict
import time
from peach.tf_nn.general import VERY_SMALL_NUMBER
...
class E2eEvaluator(object):
    ...
    def get_evaluation(self, sess, dataset_obj, data_type, global_step=None, time_counter=None, *args, **kwargs):
        ...
        if dataset_obj.data_name.startswith("e2e"):
            ...
            for _idx_ex, _example in enumerate(example_batch_list):
                ...
                if _example["question_type"] not in EO_wrt_qtype:
                    EO_wrt_qtype[_example["question_type"]] = F1MetricV2()
                    entity_type_wrt_qtype[_example["question_type"]] = F1MetricV2()
                ...
                for idx_o, (gold_ids, pred_ids, f1_metric_obj) in enumerate(zip(
                    ...
                    if idx_o < 3:
                        ...
                        EO_wrt_qtype[_example["question_type"]].add_list(zip(gold_ids, pred_ids))
                    else:
                        entity_type_f1_metric_obj.add_list(zip(gold_ids, pred_ids))
                        entity_type_wrt_qtype[_example["question_type"]].add_list(zip(gold_ids, pred_ids))

            # aggregation
            all_result = {}
            all_result["Question_Types"] = {}
            ...
            for _qt, _val in EO_wrt_qtype.items():
                if(_qt not in all_result["Question_Types"]):
                    all_result["Question_Types"][_qt] = {}
                all_result["Question_Types"][_qt].update(_val.get_metric("EO"))

            for _qt, _val in entity_type_wrt_qtype.items():
                if(_qt not in all_result["Question_Types"]):
                    all_result["Question_Types"][_qt] = {}
                all_result["Question_Types"][_qt].update(_val.get_metric("ET"))
                ...
                if tuple(_new_sketch) == tuple(_example["lf"]["gold_sketch_ids"]):
                    ...
                    if(_example["question_type"]!='Clarification'):
                        ...
                        if(_example["prev_question_type"]=='Clarification'):
                            sketch_counter_wrt_qt[_example["prev_question_type"]][1] += 1
                else:
                    sketch_counter[0] += 1
                    if(_example["question_type"]!='Clarification'):
                        sketch_counter_wrt_qt[_example["question_type"]][0] += 1
                        if(_example["prev_question_type"]=='Clarification'):
                            sketch_counter_wrt_qt[_example["prev_question_type"]][0] += 1
            ...
            for _qt, _val in sketch_counter_wrt_qt.items():
                if(_qt not in all_result["Question_Types"]):
                    all_result["Question_Types"][_qt] = {}
                all_result["Question_Types"][_qt]["sketch_accuracy"] = 1. * _val[1] / sum(_val) if sum(_val) > 0 else 0.
            ...
            accu_counter_dict_wrt_qt = {}
            for _idx_ex, _example in enumerate(example_batch_list):
                gold_label_type = _example["lf"]["gold_sketch"]
                gold_label_list =_example["lf"]["gold_leaves_ids"]

                ref_input_pos_ids = all_run_res["input_pos_ids"][_idx_ex]

                ref_input_pos_dict = {val:idx for idx, val in reversed(list(enumerate(ref_input_pos_ids)))}
                ...
                if _example["question_type"] not in accu_counter_dict_wrt_qt:
                    accu_counter_dict_wrt_qt[_example["question_type"]]= {"sketch_entity": [0, 0], "sketch_predicate": [0, 0],
                        "sketch_type": [0, 0], "sketch_num": [0, 0],}
                ...
                for _idx_tk, (_skt_type, _skt_leaf) in enumerate(zip(gold_label_type, gold_label_list)):
                    ...
                    if _skt_type == "e":
                        ...
                        gold_idx = ref_input_pos_dict[gold_idx]
                        if gold_idx == sketch_entity[_idx_tk]:
                            accu_counter_dict["sketch_entity"][1] += 1
                            if(_example["question_type"]!='Clarification'):
                                accu_counter_dict_wrt_qt[_example["question_type"]]["sketch_entity"][1] += 1
                                if(_example["prev_question_type"]=='Clarification'):
                                    accu_counter_dict_wrt_qt[_example["prev_question_type"]]["sketch_entity"][1] += 1
                        else:
                            accu_counter_dict["sketch_entity"][0] += 1
                            if(_example["question_type"]!='Clarification'):
                                accu_counter_dict_wrt_qt[_example["question_type"]]["sketch_entity"][0] += 1
                                if(_example["prev_question_type"]=='Clarification'):
                                    accu_counter_dict_wrt_qt[_example["prev_question_type"]]["sketch_entity"][0] += 1

                    elif _skt_type == "r":
                        if _skt_leaf == sketch_predicate[_idx_tk]:
                            accu_counter_dict["sketch_predicate"][1] += 1
                            if(_example["question_type"]!='Clarification'):
                                accu_counter_dict_wrt_qt[_example["question_type"]]["sketch_predicate"][1] += 1
                                if(_example["prev_question_type"]=='Clarification'):
                                    accu_counter_dict_wrt_qt[_example["prev_question_type"]]["sketch_predicate"][1] += 1
                        else:
                            accu_counter_dict["sketch_predicate"][0] += 1
                            if(_example["question_type"]!='Clarification'):
                                accu_counter_dict_wrt_qt[_example["question_type"]]["sketch_predicate"][0] += 1
                                if(_example["prev_question_type"]=='Clarification'):
                                    accu_counter_dict_wrt_qt[_example["prev_question_type"]]["sketch_predicate"][0] += 1

                    elif _skt_type == "Type":
                        if _skt_leaf == sketch_type[_idx_tk]:
                            accu_counter_dict["sketch_type"][1] += 1
                            if(_example["question_type"]!='Clarification'):
                                accu_counter_dict_wrt_qt[_example["question_type"]]["sketch_type"][1] += 1
                                if(_example["prev_question_type"]=='Clarification'):
                                    accu_counter_dict_wrt_qt[_example["prev_question_type"]]["sketch_type"][1] += 1
                        else:
                            accu_counter_dict["sketch_type"][0] += 1
                            if(_example["question_type"]!='Clarification'):
                                accu_counter_dict_wrt_qt[_example["question_type"]]["sketch_type"][0] += 1
                                if(_example["prev_question_type"]=='Clarification'):
                                    accu_counter_dict_wrt_qt[_example["prev_question_type"]]["sketch_type"][0] += 1
                        ...
                        if gold_idx == sketch_num[_idx_tk]:
                            ...
                            if(_example["question_type"]!='Clarification'):
                                accu_counter_dict_wrt_qt[_example["question_type"]]["sketch_num"][1] += 1
                                if(_example["prev_question_type"]=='Clarification'):
                                    accu_counter_dict_wrt_qt[_example["prev_question_type"]]["sketch_num"][1] += 1
                        else:
                            accu_counter_dict["sketch_num"][0] += 1
                            if(_example["question_type"]!='Clarification'):
                                accu_counter_dict_wrt_qt[_example["question_type"]]["sketch_num"][0] += 1
                                if(_example["prev_question_type"]=='Clarification'):
                                    accu_counter_dict_wrt_qt[_example["prev_question_type"]]["sketch_num"][0] += 1
            ...
            for _qt, _val in accu_counter_dict_wrt_qt.items():
                for key in _val:
                    if(_qt not in all_result["Question_Types"]):
                        all_result["Question_Types"][_qt] = {}
                    all_result["Question_Types"][_qt][key+"_accuracy"] = 1. * _val[key][1] / sum(_val[key])\
                        if sum(_val[key]) > 0 else 0.
    ...
    def get_alt_evaluation(self, sess, dataset_obj, data_type, global_step=None, time_counter=None, ent_inverse_index=None, dict_e2t=None, remapper=None, *args, **kwargs):
            assert isinstance(dataset_obj, E2eDataset)
            logging.info('getting evaluation result for %s' % data_type)
            data_name = dataset_obj.data_name
            example_batch_list = []
            vecs_dim = remapper.get_emb_dim()
            if dataset_obj.data_name.startswith("e2e"):
                all_run_res = {
                    "sep_indices": [],
                    "EOs": [],
                ...
                processor_pt = dataset_obj.processor
                ...
                for example_batch, _, _, _ in dataset_obj.generate_batch_iter(self.cfg['test_batch_size'], data_type):

                    # add portion corresponding to entity_loading
                    ...
                    run_dict = {
                        ...
                        "encoder_predicate": prediction_dict["encoder_predicate"]
                    }

                    feed_dict = self.model.get_feed_dict(example_batch, False, max_seq_len=self.cfg['max_sequence_len'])
                    run_res_np = sess.run(run_dict, feed_dict)

                    EO_label_list = dataset_obj.get_labels_dict()["EOs"]["labels"]
                    type_label_list = dataset_obj.get_labels_dict()["types"]["labels"]
                    valid_type_str_set = set(type_label_list[3:])
                    ph_tk = EMPTY_TOKEN


                    # TODO replace 160 with max_seq_length
                    res_ent_set_list, _, _, _ =get_entities_num(run_res_np, example_batch, EO_label_list,\
                        type_label_list, valid_type_str_set, ent_inverse_index, dict_e2t, None, self.cfg['max_sequence_len'], ph_tk)
    
                    
                    line_ids = [[list(each_id)[0] if each_id!=None else 'QUNKNOWN' for each_id in example] if example!=None else ['QUNKNOWN'] for example in res_ent_set_list]
                    golden_line_entities_array = get_numpy_ev(line_ids, len(example_batch))
                    

                    # evaluate decoder predicate embeddings, decoder type embeddings
                    gold_sketch_types = feed_dict[self.model.sketch_in_type]
                    gold_sketch_predicates = feed_dict[self.model.sketch_in_predicate]
                    gold_predicate_array = np.take(predicate_converter,gold_sketch_predicates)
                    gold_type_array = np.take(type_converter,gold_sketch_types)
                    
                    extras_dict = {}
                    golden_entity_inputs = remapper.get_ent_emb_matrix(golden_line_entities_array);
                    golden_type_inputs = remapper.get_type_emb_matrix(gold_type_array);
                    golden_pred_inputs = remapper.get_pred_emb_matrix(gold_predicate_array);

                    extras_dict[self.model.sketch_entity_qids] = golden_entity_inputs
                    extras_dict[self.model.sketch_type_qids] = golden_type_inputs
                    extras_dict[self.model.sketch_pred_qids] = golden_pred_inputs
                    # max_seq_len = np.max([len(each_ex) for each_ex in line_ids])
                    # assert self.model.cfg["hn"] >=  len(vecs[0]), "check TransE length" 
                    # ent_vecs = np.zeros([len(example_batch), max_seq_len+1, self.model.cfg["hn"]], np.float32)


                    # for _idx_ex in range(len(line_ids)):
                    #     for _idx_t in range(len(line_ids[_idx_ex])):
                    #         for _idx_att, _id in enumerate(line_ids[_idx_ex][_idx_t]):
                    #             ent_vecs[_idx_ex, _idx_t, _idx_att] = _id

                    flat_list = [list(set(sublist)) for sublist in line_ids]   

                    # bce_labels = np.any(np.equal(np.expand_dims(run_res_np["encoder_predicate"], -1), np.expand_dims(np.arange(self.model.num_predicate_labels), 0)), -2)
                    # pred_list = [predicate_converter[each_exp].tolist() for each_exp in bce_labels]
                    bce_labels = run_res_np["encoder_predicate"]
                    pred_list = [predicate_converter[each_exp].tolist() for each_exp in bce_labels]                 
                    one_hop_list, oh_forward_count, oh_backward_count = remapper.get_onehop(flat_list, pred_list)

                    count_forward = get_numpy_ev(oh_forward_count, len(example_batch), ["QPAD", "[PAD]", "QPAD"])
                    fe = [[each_qu[0] for each_qu in each_ex] for each_ex in count_forward ]
                    fp = [[each_qu[1] for each_qu in each_ex] for each_ex in count_forward ]
                    fe2 = [[each_qu[2] for each_qu in each_ex] for each_ex in count_forward ]
                    fc = [[0 for each_qu in each_ex] for each_ex in count_forward ]

                    
                    fe_mask = np.logical_not(np.char.equal(fe, "QPAD"))
                    fe_emb = remapper.get_ent_emb_matrix(fe)
                    fp_emb = remapper.get_pred_emb_matrix(fp)
                    fe2_emb = remapper.get_ent_emb_matrix(fe2)

                    mh_query_emb = np.concatenate([fe_emb, fe2_emb], axis=-1)

                    count_backward= get_numpy_ev(oh_backward_count, len(example_batch), ["QPAD", "[PAD]", "QPAD"])
                    bfe = [[each_qu[0] for each_qu in each_ex] for each_ex in count_backward ]
                    bfp = [[each_qu[1] for each_qu in each_ex] for each_ex in count_backward ]
                    bfe2 = [[each_qu[2] for each_qu in each_ex] for each_ex in count_backward ]
                    bfc = [[0 for each_qu in each_ex] for each_ex in count_backward ]

                    
                    bfe_mask = np.logical_not(np.char.equal(bfe, "QPAD"))
                    bfe_emb = remapper.get_ent_emb_matrix(bfe)
                    bfp_emb = remapper.get_pred_emb_matrix(bfp)
                    bfe2_emb = remapper.get_ent_emb_matrix(bfe2)

                    bmh_query_emb = np.concatenate([bfe_emb, bfe2_emb], axis=-1)


                    # one_hop_list = remapper.get_onehop(flat_list, np.take(predicate_converter, run_res_np["encoder_predicate"]))
                    # one_hop_ids = [[each_id if each_id!=None else 'QUNKNOWN' for each_id in example] if example!=None else ['QUNKNOWN'] for example in one_hop_list]
                    pred_one_hop_entities = get_numpy_ev(one_hop_list, len(example_batch))

                    one_hop_entity_mask = np.logical_not(np.char.equal(pred_one_hop_entities, "NOENT"))
                    one_hop_entity_emb = remapper.get_ent_emb_matrix(pred_one_hop_entities);

                    extras_dict[self.model.one_hop_qids] = one_hop_entity_emb
                    extras_dict[self.model.one_hop_mask] = one_hop_entity_mask

                    extras_dict[self.model.mh_queries] = mh_query_emb
                    extras_dict[self.model.mh_counts] = fc
                    extras_dict[self.model.mh_mask] = fe_mask

                    extras_dict[self.model.bmh_queries] = bmh_query_emb
                    extras_dict[self.model.bmh_counts] = bfc
                    extras_dict[self.model.bmh_mask] = bfe_mask


                    run_dict = self.run_dict
                    example_batch_list.extend(example_batch)
                    feed_dict = self.model.get_feed_dict(example_batch, False)
                    feed_dict.update(extras_dict)                    

                    run_res = sess.run(run_dict, feed_dict=feed_dict)
                    run_res["input_pos_ids"] = feed_dict[self.model.input_pos_ids]


                    for _idx_ex, _example in enumerate(example_batch):
                        input_pos_ids = run_res["input_pos_ids"][_idx_ex]
                        all_run_res["input_pos_ids"].append(list(input_pos_ids))
    ...
    def decoding(
            ...
            return_out_list=True, verbose=False, remapper=None, predicate_converter=None, type_converter=None
    ):
        valid_type_str_set = set(type_label_list[3:])
        ...
        for _idx_batch, _ex_ptr in enumerate(range(0, len(example_list), batch_size)):
            ...
            run_res["entity_set_list"], run_res["num_list"], run_res["sep_indices"], run_res["entity_set_all_list"] = \
                get_entities_num(run_res_np, _example_batch, EO_label_list, type_label_list, valid_type_str_set,\
                    ent_inverse_index, dict_e2t, alter_ner_dir, max_seq_len, ph_tk)


            line_ids = [[list(each_id)[0] if each_id!=None else 'QUNKNOWN' for each_id in example] if example!=None else ['QUNKNOWN'] for example in run_res["entity_set_list"]]
            golden_line_entities_array = get_numpy_ev(line_ids, len(_example_batch))
            

            # evaluate decoder predicate embeddings, decoder type embeddings
            # gold_sketch_types = feed_dict[self.model.sketch_type]
            # gold_sketch_predicates = feed_dict[self.model.sketch_predicate]
            # gold_predicate_array = np.take(predicate_converter,gold_sketch_predicates)
            # gold_type_array = np.take(type_converter,gold_sketch_types)
            ...
            extras_dict["entity_vecs"] = golden_entity_inputs
            extras_dict["ent_pos_dec"] = self.model.get_init_decoder_entities_pos_np(len(_example_batch))
            extras_dict["num_pos_dec"] = self.model.get_init_decoder_num_pos_np(len(_example_batch))

            type_inputs = self.model.get_init_decoder_type_np(len(_example_batch), vecs_dim)
            pred_inputs = self.model.get_init_decoder_predicate_np(len(_example_batch), vecs_dim)
            Q_type_array = np.take(type_converter,type_inputs)
            Q_predicate_array = np.take(predicate_converter,pred_inputs)
            
            extras_dict["type_vecs_dec"] = remapper.get_type_emb_matrix(Q_type_array)
            extras_dict["pred_vecs_dec"] = remapper.get_pred_emb_matrix(Q_predicate_array)
            extras_dict["type_converter"] = type_converter
            extras_dict["predicate_converter"] = predicate_converter
            extras_dict["remapper"] = remapper
            ...
            count_forward = get_numpy_ev(oh_forward_count, len(_example_batch), ["QPAD", "[PAD]", "QPAD"])
            fe = [[each_qu[0] for each_qu in each_ex] for each_ex in count_forward ]
            fp = [[each_qu[1] for each_qu in each_ex] for each_ex in count_forward ]
            fe2 = [[each_qu[2] for each_qu in each_ex] for each_ex in count_forward ]
            fc = [[0 for each_qu in each_ex] for each_ex in count_forward ]
            ...
            count_backward= get_numpy_ev(oh_backward_count, len(_example_batch), ["QPAD", "[PAD]", "QPAD"])
            bfe = [[each_qu[0] for each_qu in each_ex] for each_ex in count_backward ]
            bfp = [[each_qu[1] for each_qu in each_ex] for each_ex in count_backward ]
            bfe2 = [[each_qu[2] for each_qu in each_ex] for each_ex in count_backward ]
            bfc = [[0 for each_qu in each_ex] for each_ex in count_backward ]
            ...
            # one_hop_list = remapper.get_onehop(flat_list, np.take(predicate_converter, run_res_np["encoder_predicate"]))
            # one_hop_ids = [[each_id if each_id!=None else 'QUNKNOWN' for each_id in example] if example!=None else ['QUNKNOWN'] for example in one_hop_list]
            pred_one_hop_entities = get_numpy_ev(one_hop_list, len(_example_batch))

            one_hop_entity_mask = np.logical_not(np.char.equal(pred_one_hop_entities, "NOENT"))
            one_hop_entity_emb = remapper.get_ent_emb_matrix(pred_one_hop_entities);


            extras_dict["one_hop_qids"] = one_hop_entity_emb
            extras_dict["one_hop_mask"] = one_hop_entity_mask
            extras_dict["input_pos_ids"] = feed_dict[self.model.input_pos_ids]
            extras_dict["pos_ids_len"] = np.argmax(feed_dict[self.model.input_pos_ids], axis=-1)
            
            extras_dict["mh_queries"] = mh_query_emb
            extras_dict["mh_counts"] = fc
            extras_dict["mh_mask"] = fe_mask
            
            extras_dict["bmh_queries"] = bmh_query_emb
            extras_dict["bmh_counts"] = bfc
            extras_dict["bmh_mask"] = bfe_mask
            ...
            dec_results = logical_form_decoder(
                ...
                self.model.get_init_decoder_ids_np(len(_example_batch)), # this resturs a bs*1 vector with first id initialised to SOS
                ...
                run_res["num_list"], beam_size=4, max_depth=35, max_accu_time=timeout, extras_dict=extras_dict,
            )
...
def accumulative_eval(
        ...
        top1, pred_answer, pred_lf,
        top1_pred=None, dev_dict=None, recall=None, precision=None, ptd = None, path=None
):
    ...
    if cur_qt != 'Clarification':
        ...
        if("Simple Question (Direct)" in cur_qt):
            if(ptd!=None):    
                    ptd.write("{}\t{}\t{}\t{}\t{}\t{}\n".format(cur_qt, answer, pred_answer, pred_lf, top1, path))
...
def get_entities_num(run_res_np, _example_batch, EO_label_list, type_label_list, valid_type_str_set,\
     ent_inverse_index, dict_e2t, alter_ner_dir, max_seq_len, ph_tk):
    run_entity_set_list = [];
    run_num_list = []
    run_entity_set_all_list = []
    run_sep_list = []
    for _idx_ex, _example in enumerate(_example_batch):
        if isinstance(alter_ner_dir, str):
            ...
            sep_indices = run_res_np["sep_indices"][_idx_ex]
            sep1, sep2, sep3 = list(sep_indices)  # len is 3
            run_sep_list.append((sep1, sep2, sep3))
        ...
        for _idx_tk in range(len(ent_set_all_list)):
            ...
            # if _llen > _slen > 0:
            #     ent_set_all_list[_idx_tk] = set(list(ent_set_all_list[_idx_tk])[:_slen])  # incaseof Timeout
        # set list and set_all_list contain two lists of entities

        run_entity_set_list.append(ent_set_list)
        run_num_list.append(num_list)
        run_entity_set_all_list.append(ent_set_all_list)

    return run_entity_set_list, run_num_list, run_sep_list, run_entity_set_all_list
...
# def get_three(slf_list):
#     new_slf_list = deepcopy[]
...
def logical_form_decoder(
    ...
    bs_entity_vecs = extras_dict["entity_vecs"]
    entity_vecs = np.copy(extras_dict["entity_vecs"])
    entity_pos_dec_np = extras_dict["ent_pos_dec"]
    num_pos_dec_np = extras_dict["num_pos_dec"]
    type_vecs_dec_np = extras_dict["type_vecs_dec"]
    pred_vecs_dec_np = extras_dict["pred_vecs_dec"]
    type_converter = extras_dict["type_converter"]
    predicate_converter = extras_dict["predicate_converter"]
    remapper = extras_dict["remapper"]
    bs_input_pos_ids = extras_dict["input_pos_ids"]
    bs_pos_ids_len = extras_dict["pos_ids_len"]

    mh_queries_np = np.copy(extras_dict["mh_queries"])
    mh_counts_np = np.copy(extras_dict["mh_counts"])
    mh_mask_np = np.copy(extras_dict["mh_mask"])

    bs_mh_queries = np.copy(extras_dict["mh_queries"])
    bs_mh_counts = np.copy(extras_dict["mh_counts"])
    bs_mh_mask = np.copy(extras_dict["mh_mask"])

    bmh_queries_np = np.copy(extras_dict["bmh_queries"])
    bmh_counts_np = np.copy(extras_dict["bmh_counts"])
    bmh_mask_np = np.copy(extras_dict["bmh_mask"])

    bs_bmh_queries = np.copy(extras_dict["bmh_queries"])
    bs_bmh_counts = np.copy(extras_dict["bmh_counts"])
    bs_bmh_mask = np.copy(extras_dict["bmh_mask"])



    in_pos_ids_np = np.copy(bs_input_pos_ids)

    bs_oh_entity_vecs = np.copy(extras_dict["one_hop_qids"])
    bs_oh_entity_masks = np.copy(extras_dict["one_hop_mask"])

    oh_entity_vecs_np = np.copy(extras_dict["one_hop_qids"])
    oh_entity_masks_np = np.copy(extras_dict["one_hop_mask"])
    ...
    encoder_ids_np = np.array(encoder_ids_np_init, "int32")
    decoder_history_np = decoder_history_np_init # np.zeros matrix bs*num_layer*0*hn
    ...
    cas_point_np=list(range(bs_init))
    _cur_depth = 0
    while True:
        _cur_depth += 1
        feed_dict = {
            ...
            model_dec_dict["sketch_entity_qids"]: entity_vecs, 
            model_dec_dict["sketch_entity_pos"]: entity_pos_dec_np, 
            model_dec_dict["sketch_num_pos"]: num_pos_dec_np, 
            model_dec_dict["sketch_type_qids"]: type_vecs_dec_np, 
            model_dec_dict["sketch_pred_qids"]: pred_vecs_dec_np, 
            model_dec_dict["one_hop_qids"]: oh_entity_vecs_np, 
            model_dec_dict["one_hop_mask"]: oh_entity_masks_np, 
            model_dec_dict["mh_queries"]: mh_queries_np, 
            model_dec_dict["mh_counts"]: mh_counts_np, 
            model_dec_dict["mh_mask"]: mh_mask_np,
            model_dec_dict["bmh_queries"]: bmh_queries_np, 
            model_dec_dict["bmh_counts"]: bmh_counts_np, 
            model_dec_dict["bmh_mask"]: bmh_mask_np, 
            model_dec_dict["input_pos_ids_placeholder"]: in_pos_ids_np,
        }
        ...
        for _idx_ex in range(bs_init):
            num_ex_tokens = bs_pos_ids_len[_idx_ex]  # token level indexes
                ...
                for _top_idx, _top_score in zip(_top_indices, _top_scores):
                    ...
                    if _new_token == "e":  # sketch is "e", fetch the candidate entities
                        ...
                        for _idx_i, (_ent_idx, _ent_score) in enumerate(zip(_ent_idxs, _ent_scores)):
                            _ent_idx_t = bs_input_pos_ids[_idx_ex][_ent_idx]
                            _ent_set = ent_set_lists[_idx_ex][_ent_idx_t]
                        ...
                        if len(_candi_ent_sets) > 0:
                            ...
                            for _ent_set, _ent_score, _ent_pos in zip(_candi_ent_sets, _candi_ent_scores, _cand_ent_pos):
                                ...
                                _tmp_slfs.append(deepcopy(_slf) + [("e", _ent_set, _flag, _ent_pos)])
                                ...
                                # _new_lf_elem = ("e", _ent_set, _flag, _ent_pos)
                    elif _new_token == "r":
                        ...
                        for _num_idx, _num_score in zip(_num_idxs, _num_scores):
                            _num_idx_t = bs_input_pos_ids[_idx_ex][_num_idx]
                            _num = num_lists[_idx_ex][_num_idx_t]
                            if _num is None:
                                continue
                            ...
                            if _num not in _candi_num_list:
                                ...
                                _cand_num_pos.append(_num_idx)
                            if len(_candi_num_list) > _n_beam_num:
                                break
                        if len(_candi_num_list) > 0:
                            _candi_num_scores = score_list_norm(_candi_num_scores)
                            for _num, _num_score, _num_pos in zip(_candi_num_list, _candi_num_scores, _cand_num_pos):
                                _new_lf_elem = ("num_utterence", _num, False)
                                _tmp_slfs.append(deepcopy(_slf) + [("num_utterence", _num, False, _num_pos)])
                                _tmp_answers.append(deepcopy(_answer) + [_new_lf_elem])
        ...
        assert bs_init == bs_entity_vecs.shape[0]
        assert bs_init == bs_mh_queries.shape[0]
        for _idx_ex in range(bs_init):
            for _candi_sketch in slfs_list[_idx_ex]:
                ...
                entity_vecs_arr.append(bs_entity_vecs[_idx_ex])

                _ent_pos_id = _lf_elem[-1] if _lf_elem[0] == "e" else -1
                _num_pos_id = _lf_elem[-1] if _lf_elem[0] == "num_utterence" else -1
                entity_pos_inputs.append(_ent_pos_id)
                num_pos_inputs.append(_num_pos_id)


                _type_id = _lf_elem[1] if _lf_elem[0] == "Type" else '[PAD]'
                _pred_id = _lf_elem[1] if _lf_elem[0] == "r" else '[PAD]'
                type_inputs.append([_type_id])
                pred_inputs.append([_pred_id])

                mh_queries_array.append(bs_mh_queries[_idx_ex])
                mh_count_array.append(bs_mh_counts[_idx_ex])
                mh_mask_array.append(bs_mh_mask[_idx_ex])

                bmh_queries_array.append(bs_bmh_queries[_idx_ex])
                bmh_count_array.append(bs_bmh_counts[_idx_ex])
                bmh_mask_array.append(bs_bmh_mask[_idx_ex])



                oh_entity_vecs.append(bs_oh_entity_vecs[_idx_ex])
                oh_entity_masks.append(bs_oh_entity_masks[_idx_ex])

                in_pos_ids_array.append(bs_input_pos_ids[_idx_ex])
                tester_array.append(_idx_ex)
                # if(_lf_elem[0] == 'e'):
                #     entity_pos_inputs.append(_lf_elem[2])
                # else:
                #     entity_pos_inputs.append(-1)
        ...
        if len(decoder_inputs) > 0:
            ...
            decoder_ids_np = np.array(decoder_inputs, dtype=np.int32).reshape([len(decoder_inputs), 1])
            # 5. decoder_ids: decoder_ids_np
            entity_pos_dec_np = np.array(entity_pos_inputs, dtype=np.int32).reshape([len(entity_pos_inputs), 1])
            num_pos_dec_np = np.array(num_pos_inputs, dtype=np.int32).reshape([len(entity_pos_inputs), 1])
            entity_vecs = np.array(entity_vecs_arr, dtype=np.float32)

            # Q_type_array = np.take(type_converter,type_inputs)
            # Q_predicate_array = np.take(predicate_converter,pred_inputs)

            type_vecs_dec_np = remapper.get_type_emb_matrix(type_inputs);
            pred_vecs_dec_np = remapper.get_pred_emb_matrix(pred_inputs);

            mh_queries_np = np.array(mh_queries_array, dtype=np.float32)
            mh_counts_np = np.array(mh_count_array, dtype=np.int32)
            mh_mask_np = np.array(mh_mask_array, dtype=bool)

            bmh_queries_np = np.array(bmh_queries_array, dtype=np.float32)
            bmh_counts_np = np.array(bmh_count_array, dtype=np.int32)
            bmh_mask_np = np.array(bmh_mask_array, dtype=bool)

            oh_entity_vecs_np = np.array(oh_entity_vecs, dtype=np.float32)
            oh_entity_masks_np = np.array(oh_entity_masks, dtype=bool)

            in_pos_ids_np = np.array(in_pos_ids_array, dtype=np.int32)
            cas_point_np = np.stack([cas_point_np[_ptr] for _ptr in new_branch_ptrs_list], axis=0)
            if(not np.array_equal(tester_array, cas_point_np)):
                print("Difference", tester_array, cas_point_np, "\n\n\n")

        else:  # stop the beam search
            break
        end_analysis = time.time()
        t2 = end_analysis-start_analysis
        t1 = start_analysis -start_tf
    # !!! end of beam search
    for _idx_ex in range(bs_init):
